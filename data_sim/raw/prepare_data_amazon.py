...
def parse_zipped_json(path):
    unzipped_json = gzip.open(path, 'rb')
    for record in unzipped_json:
        yield json.loads(record.decode("utf-8"))


def get_raw_data(path):
    raw_dict = {}
    for i, record in enumerate(parse_zipped_json(path)):
        raw_dict[i] = record
    return pd.DataFrame.from_dict(raw_dict, orient='index')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, default="toys_5",
        help="specify the dataset to be preprocessed")
    parser.add_argument("--source", type=str, choices=["user"],
        default="user", help="use vae to embed users or items")
    parser.add_argument("--min_ucount", type=int, default=10,
        help="keep users who clicked on at least min_ucount items")
    parser.add_argument("--min_icount", type=int, default=10,
    ...
    raw_path = os.path.join(args.dataset, "json.gz")
    raw_data = get_raw_data(raw_path)
    raw_data = raw_data[["reviewerID", "asin", "overall"]]
    raw_data = raw_data.rename(columns={"reviewerID":"uid", "asin":"vid", "overall":"rating"})
    ...
    if args.simulate == "ratings":
        ...
        filtered_data = filtered_data[filtered_data.vid.isin(remaining_vids)]
    else:
